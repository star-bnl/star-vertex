...
void StGenericVertexFinder::UseVertexConstraint(const vertexSeed_st& beamline)
{
   ...
   LOG_INFO << "x(z) = (" << sBeamline.x0 << " +/- max(0.01, " << sBeamline.err_x0 << ") ) + (" << sBeamline.dxdz << " +/- max(0.0001, " << sBeamline.err_dxdz << ") ) * z" << endm;
   LOG_INFO << "y(z) = (" << sBeamline.y0 << " +/- max(0.01, " << sBeamline.err_y0 << ") ) + (" << sBeamline.dydz << " +/- max(0.0001, " << sBeamline.err_dydz << ") ) * z" << endm;
